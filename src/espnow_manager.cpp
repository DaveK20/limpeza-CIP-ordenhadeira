...
static int findByMac(const uint8_t *mac)
{
    for (int i = 0; i < espnow_moduleCount; i++)
        if (macEqual(espnow_modules[i].mac, mac))
            return i;
    return -1;
}
...
{
    for (int i = 0; i < espnow_moduleCount; i++)
        if (strcmp(espnow_modules[i].name, name) == 0)
            return i;
    return -1;
}
...
static void checkAllReady()
{
    ...
    for (int i = 0; i < REQUIRED_MODULE_COUNT; i++)
    {
        ...
        if (idx == -1 || !espnow_modules[idx].active)
        {
            ok = false;
            break;
        }
...
String espnow_getStatusJson()
{
    ...
    for (int i = 0; i < REQUIRED_MODULE_COUNT; i++)
    {
        ...
        json += "{\"name\":\"" + String(REQUIRED_MODULES[i]) + "\"," + "\"active\":" + (active ? "true" : "false") + "}";
    }
